  ...
  return (
    <div className="flex-1 overflow-y-auto" style={{ background: "#F3F0F9" }}>
      <div className="px-4 pt-2 pb-4">
        <div className="text-xs font-medium" style={{ color: "#49454F" }}>
          MPLADS AI-GUARDIAN
        </div>
        <div className="text-2xl font-semibold" style={{ fontFamily: "'Google Sans Display', sans-serif", color: "#1C1B1F" }}>
          Field Inspector
        </div>
        <div className="text-xs mt-1" style={{ color: "#79747E" }}>
          {assignments.length} assigned field {assignments.length === 1 ? "case" : "cases"}
        </div>
      </div>
              ...
              <div className="p-5 space-y-3">
                <div className="flex items-center gap-3">
                  <div
                    ...
                    style={{ background: project.status === "HIGH RISK" ? "#FFDAD6" : "#FFEFD6" }}
                  >
                    <span className="text-lg font-black" style={{ color: project.status === "HIGH RISK" ? "#B3261E" : "#7C4F00" }}>
                      {project.risk}
                    </span>
                  </div>
                  <div className="min-w-0">
                    <div className="font-semibold text-sm truncate" style={{ color: "#1C1B1F" }}>
                      {project.title}
                    </div>
                    <div className="text-[10px] font-mono" style={{ color: "#79747E" }}>
                      {project.id}
                    </div>
                  </div>
                </div>
                ...
                {[
                  ...
                  <div key={row.l} className="flex items-start justify-between gap-2">
                    <span className="text-[10px] font-medium shrink-0" style={{ color: "#79747E" }}>
                      {row.l}
                    </span>
                    <span className="text-xs font-medium text-right" style={{ color: "#1C1B1F" }}>
                      {row.v}
                    </span>
                  </div>
                ))}
                <div className="flex items-center justify-between gap-2 pt-1">
                  <span className="text-[10px] font-semibold px-2.5 py-1 rounded-full" style={{ background: "#FFEFD6", color: "#7C4F00" }}>
                    Awaiting Field Verification
                  </span>
                  <button
                    onClick={() => handleBeginVerification(project)}
                    className="h-9 px-4 rounded-full text-xs font-semibold text-white md-ripple"
      ...
      {state !== "project" && !selectedProject && (
        <div className="px-4">
          <Card>
            <div className="p-6 text-center text-sm" style={{ color: "#49454F" }}>
              No field case selected.
            </div>
          </Card>
        </div>
      )}
        ...
        <div className="px-4 space-y-4 animate-scale-in">
          ...
          <Card>
            <div className="p-4">
              ...
              <div className="text-sm font-semibold mt-1" style={{ color: "#1C1B1F" }}>
      ...
      {state === "exif" && selectedProject && (
        <div className="px-4 space-y-4 animate-fade-in">
          <Card>
            <div className="p-5">
              <div className="text-[10px] font-mono mb-1" style={{ color: "#79747E" }}>
                {selectedProject.id}
              </div>
              <div className="text-sm font-semibold mb-4" style={{ color: "#1C1B1F" }}>
                Extracting Metadata...
              </div>
              <div className="space-y-3">
                {exifSteps.map((step, index) => (
                  <div
                    ...
                    style={{ color: index < exifStep ? "#006C4C" : "#CAC4D0" }}
                  >
                    <div
                      className="w-6 h-6 rounded-full flex items-center justify-center shrink-0"
                      style={{ background: index < exifStep ? "#D4F8E8" : "#ECE6F0" }}
                    >
                      <span className="text-xs font-bold" style={{ color: index < exifStep ? "#006C4C" : "#CAC4D0" }}>
                        {index < exifStep ? "✓" : index + 1}
                      </span>
                    </div>
                    {step}
                  </div>
            ...
            <div className="p-4 space-y-2 font-mono">
              {[
                ...
                <div key={row.l} className="flex justify-between gap-3">
                  ...
                  <span className="text-[10px] font-semibold text-right" style={{ color: row.vColor }}>
                    {row.v}
                  </span>
                </div>
              ))}
      ...
      {state === "pfms" && selectedProject && (
        <div className="px-4 space-y-4 animate-scale-in">
          <div className="rounded-3xl p-8 text-center" style={{ background: "#D4F8E8" }}>
            <div className="w-16 h-16 rounded-full mx-auto mb-4 flex items-center justify-center text-2xl" style={{ background: "#006C4C", color: "#FFFFFF" }}>
              ✓
            </div>
            <div className="text-base font-bold mb-1" style={{ color: "#006C4C" }}>
            ...
            <div className="text-xs mt-1" style={{ color: "#49454F" }}>
              PFMS payment release triggered for the selected field case.
            </div>
          </div>
          <Card>
            <div className="p-4 space-y-2 font-mono">
              {[
                ...
                <div key={row.l} className="flex justify-between gap-3">
                  ...
                  <span className="text-[10px] font-bold text-right" style={{ color: row.vc }}>
                    {row.v}
                  </span>
                </div>
              ))}
